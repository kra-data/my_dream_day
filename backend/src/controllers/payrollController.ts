...
export const exportPayrollXlsx: (req: AuthRequiredRequest, res: Response) => Promise<void> =
  async (req, res) => {
    ...
    const byEmp = new Map<bigint, { minutes: number; gross: number }>();
...
const shopId = asBigInt(req.params.shopId, 'shopId');
  ...
  const empMap = new Map<bigint, { grossTotal: number; hasUnsettled: boolean }>();
    ...
    for (const emp of employees) {
      try {
        ...
        created.push({
          employeeId: emp.id,
          name: emp.name,
          payUnit: emp.payUnit?"HOURLY":null,
          workedMinutes,
          basePay,
          totalPay,
          incomeTax,
...
const shopId = asBigInt(req.params.shopId, 'shopId');
  ...
  const aggByEmp = new Map<bigint, { workedMinutes: number; finalPayAmount: number }>();
  ...
  const paidSet = new Map<bigint, { id: bigint; settledAt: Date | null }>();
  ...
  const settledAmtByEmp = new Map<bigint, number>();
